#!/usr/bin/env python3
"""Run pure Stage C protocol-v2/session-outbox fixtures."""
...
MODULES = {
    ...
    "QuestProtocol": SRC / "ReplicatedStorage/Shared/Quest/QuestProtocol.lua",
    "Quest.QuestSessionOutbox": SRC / "ServerScriptService/Services/Quest/QuestSessionOutbox.lua",
    "Quest.QuestProtocolServer": SRC / "ServerScriptService/Services/Quest/QuestProtocolServer.lua",
}
TEST = ROOT / "tools/quest_protocol_test.luau"
...
def main() -> int:
    for path in [*MODULES.values(), TEST]:
        if not path.exists():
            sys.exit(f"missing Stage C protocol test input: {path}")
